...
// ══════════════════════════════════════════════════
const CONFIG = {
  GAS_URL: 'https://script.google.com/macros/s/AKfycbwb09h1qwFgpCZ4RSTIcnykd9sUfYI7J56ovNCmC1UcrD5357kjvz0U7ssbAvD3GvsR/exec',
  METEO: { DEFAULT_LAT: 44.8378, DEFAULT_LON: -0.5792 },
  IDB: {
    NAME: 'sportsync', VERSION: 5,
...
function gasRequest(method, body, params) {
  ...
  return $.ajax({
    url: CONFIG.GAS_URL, method: 'POST', contentType: 'text/plain',
...
async function importRemoteData(remote) {
  for (const d of (remote.dispos || [])) {
    ...
    await idbPut('dispos', {
      id: ck, _compositeKey: ck, name: d.name || '', date: d.date || '',
  ...
  if (remote.session) {
    ...
    await idbPut('session', {
      id: 'current', date: s.date || '', venue: s.venue || '',
...
function exportICS() {
  ...
  if (!s?.date) {
    showToast("Renseignez d'abord la date", 'error');
    return;
  }

  // 1. Gestion de la date (on reste en format "Local" pour éviter les mauvaises surprises de fuseau horaire)
  const dtStr = s.date.length > 10 ? s.date : s.date + 'T00:00';
  const dt = new Date(dtStr);
  const dtEnd = new Date(dt.getTime() + 3600000); // +1 heure

  // Formateur de date ICS Local (Ex: 20260518T200000)
  const fmt = d => {
    const pad = n => String(n).padStart(2, '0');
    return `${d.getFullYear()}${pad(d.getMonth() + 1)}${pad(d.getDate())}T${pad(d.getHours())}${pad(d.getMinutes())}${pad(d.getSeconds())}`;
  };

  // 2. Nettoyage et échappement du texte (virgules, points-virgules et sauts de ligne)
  const escapeText = (str) => {
    ...
    return str
      ...
      .replace(/;/g, '\\;')
      .replace(/,/g, '\\,')
      .replace(/\n/g, '\\n'); // Convertit les vrais retours à la ligne en \n textuel
  };

  const playersList = state.players ? state.players.map(p => p.name).join(', ') : '';
  const descriptionRaw = `Inscrits : ${playersList}\n${s.notes || ''}`;

  // 3. Fonction pour découper les lignes à 75 caractères maximum (Spécif ICS)
  const foldLine = (line) => {
    const parts = [];
    while (line.length > 75) {
      parts.push(line.substring(0, 75));
      line = ' ' + line.substring(75); // Les lignes suivantes DOIVENT commencer par un espace
    }
    parts.push(line);
    return parts.join('\r\n');
  };

  // 4. Construction des propriétés
  const properties = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//SportSync//FR',
    ...
    'METHOD:PUBLISH',
    'BEGIN:VEVENT',
    `UID:sportsync-${state.sessionId || 'session'}-${Date.now()}@app`, // UID unique
    `DTSTART:${fmt(dt)}`,
    `DTEND:${fmt(dtEnd)}`,
    foldLine(`SUMMARY:${escapeText(`Session ${s.sport || 'Sport'} — ${s.venue || 'SportSync'}`)}`),
    foldLine(`DESCRIPTION:${escapeText(descriptionRaw)}`),
    foldLine(`LOCATION:${escapeText(s.address || s.venue || '')}`),
    'STATUS:CONFIRMED',
    'END:VEVENT',
    'END:VCALENDAR'
  ];

  // Jointure stricte avec \r\n
  const icsContent = properties.join('\r\n');
  ...
  document.body.appendChild(a); // Recommandé pour la compatibilité mobile
  a.click();
...
async function saveSession() {
  ...
  const session = {
    id: 'current', date: $('#session-date').val(), venue: $('#session-venue').val(),
  ...
  gasWrite('createSession', {
    sessionId: state.sessionId, sport: session.sport, status: 'open',
...
function formatDate(dateStr) {
  if (!dateStr) return '—';
  try {
    const dp = dateStr.includes('T') ? dateStr.split('T')[0] : dateStr;
