/**
 * ════════════════════════════════════════════════════════════════
 * SPORTSYNC — app.js  v5  (jQuery)
 * ════════════════════════════════════════════════════════════════
 * Améliorations v5 :
 *   1. Navigation mémorisée dans sessionStorage (vue + étape)
 *      → reload = même page qu'avant
 *   2. Footer sync global sur toutes les vues
 *      → heure dernière sync + bouton "Actualiser"
 *   3. Profil instantané (email + pseudo) sans reload
 *   4. syncFromSheets() "douce" : ne remplace pas les dispos
 *      locales en cours d'envoi (délégué à dispo.js)
 *   5. Auto-sync 30s sur les données autres que dispos
 *      (slots, players, session) — les dispos sont gérées par dispo.js
 */
...
const CONFIG = {
  GAS_URL: 'https://script.google.com/macros/s/AKfycbzl87WUDXV485N0YsLxy0VaS96BpLr-Gc5vh9x8_LWMHZHK488Voa3QUbDwXU7m2kvR/exec',
...
const state = {
  ...
  lastSyncTs: 0,  // timestamp de la dernière synchro réussie
};
...
/**
 * Synchronisation complète depuis GAS.
 * Les dispos ne sont pas remplacées : dispo.js gère sa propre sync douce.
 */
...
async function importRemoteData(remote) {
  // ── Dispos : merge plutôt qu'écraser (ne pas perdre les pending locaux) ──
  // On n'appelle PAS idbClear('dispos') pour préserver les données locales en attente
...
async function loadStateFromIDB() {
  ...
  renderPlayers();
  renderSession();
  if(window.SportSyncDispo) window.SportSyncDispo.refresh(true); // preserve local
...
// ══════════════════════════════════════════════════
// 6. FOOTER SYNC — heure + bouton actualiser
// ══════════════════════════════════════════════════

/**
 * Met à jour le footer de sync sur toutes les vues.
 * Affiche : "Dernière mise à jour : HH:MM:SS" + bouton "Actualiser"
 */
...
// ══════════════════════════════════════════════════
// 7. NAVIGATION — mémorisée dans sessionStorage
// ══════════════════════════════════════════════════

const NAV_KEY = 'sportsync_nav'; // clé sessionStorage

/**
 * Sauvegarde la vue + étape courante dans sessionStorage.
 * sessionStorage est effacé à la fermeture de l'onglet mais persiste au F5.
 */
function saveNavState(view, step) {
  try {
    sessionStorage.setItem(NAV_KEY, JSON.stringify({ view: view||state.currentView, step: step||state.currentStep }));
  } catch(e) {}
}
...
function showView(viewName, updateHistory) {
  ...
  // Mettre à jour l'URL sans recharger (pour un vrai deep-link)
...
function goToStep(n) {
  ...
  state.currentStep=n;
  saveNavState(state.currentView, n);
  window.scrollTo({top:0,behavior:'smooth'});
}

// ══════════════════════════════════════════════════
// 8. PROFIL — application instantanée
// ══════════════════════════════════════════════════

/**
 * Lit email + pseudo depuis localStorage et les applique partout
...
function applyProfile(name, email) {
  ...
  // Sync dans tous les champs du DOM
  $('#dispo-username').val(finalName).trigger('change'); // met à jour DS.userName dans dispo.js
  ...
  // Recharger les matchs si email disponible
...
function parseURLParams() {
  ...
  // Déterminer la vue et l'étape à afficher
  // Priorité : URL > sessionStorage > défaut dashboard
  ...
  if(['dashboard','clubs','profile','session'].includes(targetView)){
    showView(targetView, false); // false = ne pas re-écrire l'URL
    if(targetView==='session') {
      // Restaurer l'étape après un court délai (DOM pas encore prêt)
...
async function renderWeather(date){
  ...
  try{const w=await fetchWeather(date);const rain=w.rain>5?'🌧':w.rain>0?'🌦':'☀️';
    $c.html(`<div class="weather-grid"><div class="weather-cell"><div class="weather-temp">${w.tempMax??'--'}°</div><div class="weather-label">Max</div></div><div class="weather-cell"><div class="weather-temp" style="color:var(--text-sub)">${w.tempMin??'--'}°</div><div class="weather-label">Min</div></div><div class="weather-cell"><div class="weather-temp" style="font-size:1.6rem">${rain}</div><div class="weather-label">${w.rain??0} mm</div></div><div class="weather-cell"><div class="weather-temp" style="font-size:1rem;color:var(--accent3)">${w.wind??'--'}</div><div class="weather-label">km/h vent</div></div></div>`);}
  catch{$c.html('<p class="empty-state">Météo non disponible.</p>');}
}
...
function exportICS(){
  ...
  showToast('Fichier calendrier ✓','success');
}
...
function renderSlots(){
  ...
  $c.html(state.slots.map(slot=>`<div class="slot-item"><div class="slot-info"><div class="slot-date">${formatDate(slot.date)} · ${slot.start}${slot.end?' – '+slot.end:''}</div><div class="slot-meta">${slot.venue||'Lieu non précisé'}</div></div><span class="slot-price">${slot.price||'—'}</span><button class="vote-btn ${slot._voted?'voted':''}" data-id="${slot.id}" ${state.isOffline?'disabled':''}>👍 ${slot.votes||0}</button></div>`).join(''));
...
$(document).on('click','.club-suggestion',function(){
  try{const c=JSON.parse($(this).attr('data-club').replace(/&#39;/g,"'"));
    $('#session-venue').val(c.name||'');$('#session-address').val(c.address||'');
    $('#session-maps-url').val(c.mapsUrl||'');$('#session-booking-url').val(c.bookingUrl||'');
    $('#session-club-search').val(c.name||'');$('#club-suggestions').addClass('hidden');
...
async function voteSlot(id){
  ...
  slot._voted=!slot._voted;const delta=slot._voted?1:-1;slot.votes=(slot.votes||0)+delta;
  await idbPut('slots',slot);if(slot._voted&&slot.date)renderWeather(slot.date);renderSlots();
...
async function saveSession(){
  const session={id:'current',date:$('#session-date').val(),venue:$('#session-venue').val(),
    ...
    notes:$('#session-notes').val(),maxPlayers:Number($('#session-max-players').val())||10,sport:$('#session-sport').val()||''};
...
function bindEvents(){
  ...
  // Profil — application instantanée
...
async function boot(){
  try{
    state.db=await initIDB();console.log('[Boot] IDB v4');
    parseURLParams();           // restaure vue + étape depuis sessionStorage
    await loadStateFromIDB();   // données locales immédiates
    applyProfile();             // applique le profil sauvegardé sans reload
    ...
    initClubSearch();
    updateSyncFooter();
    console.log('[Boot] SportSync v5 ✓',{view:state.currentView,step:state.currentStep});
  }catch(err){
